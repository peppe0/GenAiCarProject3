...
{
    [SerializeField] private float m_StepPenalty = 0.0005f;
    [SerializeField] private float m_DistanceRewardWeight = 0.01f;
    ...
    int m_Selection;
    float m_PreviousDistanceToGoal;
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        if (useVectorObs)
        {
            // Non-directional observations only
            sensor.AddObservation(StepCount / (float)MaxStep);
            // Add scalar distance to goal (no direction information)
            float distanceToGoal = Vector3.Distance(transform.position, GetCorrectGoal().position);
    ...
    Transform GetCorrectGoal()
    {
        return m_Selection == 0 ? symbolOGoal.transform : symbolXGoal.transform;
    }
...

    {
        ...
        var currentDistanceToGoal = Vector3.Distance(transform.position, GetCorrectGoal().position);
        AddReward((m_PreviousDistanceToGoal - currentDistanceToGoal) * m_DistanceRewardWeight);
        m_PreviousDistanceToGoal = currentDistanceToGoal;
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        m_PreviousDistanceToGoal = Vector3.Distance(transform.position, GetCorrectGoal().position);
